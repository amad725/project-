...
*******************************************************************************/
	...
	public void doMutation(){
		...
		case 4:     //  Mutation for Scheduling Problem
				...
				for (int g = 0 ; g < Parameters.numGenes ;  g++){
					...
					for (int b=0; b < Parameters.geneSize; b++){
						val = 2 * val;
						if (this.chromo.charAt(Parameters.geneSize * g + b) == '1') 
							val = val + 1;
						
						// Additive Mutation
						val = val + (int)((g+1) * Math.random()); 
					}
					val = val % (g+1); 
				}
...
*******************************************************************************/
	...
	public static int selectParent(){
		...
		case 4:    // Rank Selection
			...
			double temp;
			int rank = Parameters.popSize; 
			
			randnum     = Search.r.nextDouble();
			temp        =  randnum * Parameters.popSize * (Parameters.popSize + 1)/2;
			while(temp < rank*(rank+1)/2)
				rank--;
			
			return rank;          // Select individual ranked at 'rank'
	...
	private static void sortPopulation(){
		Chromo Tmember = new Chromo();
		//  Bubble Sorting  the members Search.members[] in the order of decreasing fitness
		for (int i=Parameters.popSize-1; i>0; i--){
			for (int j=0; j<i; j++){
				//if ( Search.problem.doRawFitness(Search.member[j]) > Search.problem.doRawFitness(member[j+1])){
				if (  Search.member[j].rawFitness  > Search.member[j+1].rawFitness ){
					Tmember = Search.member[j];
					Search.member[j] = Search.member[j+1];
					Search.member[j+1] = Tmember;
				}
			}
		}
	}
